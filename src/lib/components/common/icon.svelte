<script lang="ts">
  import { CircleArrowRight, LoaderCircle, Users, Cog, Accessibility, MonitorSmartphone, ChevronDown, Menu } from "@lucide/svelte";
  import { type IIcon } from "@payload-types";

  const { icon, name, class: className }: { icon?: IIcon; name?:string, class?: string } = $props();
